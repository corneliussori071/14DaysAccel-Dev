...
function leaderboardBanner(referralLink: string): string {
  return `<!-- 14DaysAccel Dev - Leaderboard Banner (728x90) - Ad -->
<div style="width:728px;max-width:100%;height:90px;position:relative;overflow:hidden;border-radius:8px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;cursor:pointer" onclick="window.open('${referralLink}','_blank')">
  ...
  <div style="position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;height:100%;padding:0 24px">
    <div>
      <div style="color:#fff;font-size:18px;font-weight:700;letter-spacing:-0.5px">Production-Ready Software in 14 Days</div>
      <div style="color:#a1a1aa;font-size:12px;margin-top:2px">Starting at $200. Skip months of development.</div>
    </div>
    <div style="background:#fff;color:#18181b;padding:10px 20px;border-radius:6px;font-size:13px;font-weight:600;white-space:nowrap;transition:transform 0.2s;animation:btnPulse 2s ease-in-out infinite">Get Started</div>
  </div>
...
function mediumRectBanner(referralLink: string): string {
  return `<!-- 14DaysAccel Dev - Medium Rectangle (300x250) - Ad -->
<div style="width:300px;height:250px;position:relative;overflow:hidden;border-radius:10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;cursor:pointer" onclick="window.open('${referralLink}','_blank')">
  ...
  <div style="position:relative;z-index:1;display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;padding:24px;text-align:center">
    <div style="color:#38bdf8;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px;animation:fadeSlideUp 0.8s ease-out">14DaysAccel Dev</div>
    <div style="color:#fff;font-size:22px;font-weight:700;line-height:1.2;margin-bottom:6px;animation:fadeSlideUp 0.8s ease-out 0.1s both">Ship Software<br>in 14 Days</div>
    <div style="color:#94a3b8;font-size:12px;margin-bottom:20px;animation:fadeSlideUp 0.8s ease-out 0.2s both">From idea to production. Starting at $200.</div>
    <div style="background:#fff;color:#0f172a;padding:10px 28px;border-radius:6px;font-size:13px;font-weight:600;animation:fadeSlideUp 0.8s ease-out 0.3s both,btnGlow 2s ease-in-out infinite">Learn More</div>
  </div>
...
function skyscraperBanner(referralLink: string): string {
  return `<!-- 14DaysAccel Dev - Skyscraper (160x600) - Ad -->
<div style="width:160px;height:600px;position:relative;overflow:hidden;border-radius:10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;cursor:pointer" onclick="window.open('${referralLink}','_blank')">
  ...
  <div style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;justify-content:space-between;height:100%;padding:40px 14px 30px;text-align:center">
    <div>
      <div style="color:#f59e0b;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;animation:fadeIn 0.6s ease-out">14DaysAccel Dev</div>
      <div style="color:#fff;font-size:20px;font-weight:700;line-height:1.2;margin-top:16px;animation:fadeIn 0.6s ease-out 0.15s both">Build<br>Software<br>Faster</div>
      <div style="width:30px;height:2px;background:#f59e0b;margin:16px auto;animation:fadeIn 0.6s ease-out 0.3s both"></div>
      <div style="color:#a1a1aa;font-size:11px;line-height:1.5;animation:fadeIn 0.6s ease-out 0.4s both">Production-ready apps delivered in under 14 days</div>
    </div>
    <div>
      <div style="color:#f59e0b;font-size:32px;font-weight:800;animation:fadeIn 0.6s ease-out 0.5s both">$200</div>
      <div style="color:#71717a;font-size:10px;margin-top:2px;animation:fadeIn 0.6s ease-out 0.55s both">Starting price</div>
      <div style="background:#f59e0b;color:#18181b;padding:10px 24px;border-radius:6px;font-size:12px;font-weight:700;margin-top:16px;animation:fadeIn 0.6s ease-out 0.6s both">Get Started</div>
    </div>
  </div>
...
function mobileBanner(referralLink: string): string {
  return `<!-- 14DaysAccel Dev - Mobile Banner (320x50) - Ad -->
<div style="width:320px;max-width:100%;height:50px;position:relative;overflow:hidden;border-radius:6px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;cursor:pointer" onclick="window.open('${referralLink}','_blank')">
  ...
  <div style="position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;height:100%;padding:0 12px 0 32px">
    <div>
      <div style="color:#fff;font-size:13px;font-weight:700">Software in 14 Days</div>
      <div style="color:#94a3b8;font-size:10px">From $200. Ship faster.</div>
    </div>
    <div style="background:#0ea5e9;color:#fff;padding:6px 14px;border-radius:4px;font-size:11px;font-weight:600;white-space:nowrap;animation:mobileGlow 2s ease-in-out infinite">Try Now</div>
  </div>
...
export default function BannerAssets({ referralLink }: BannerAssetsProps) {
  const banners = [
    {
